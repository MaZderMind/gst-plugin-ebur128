...
static GstElement *setup_element(const gchar *caps_str) {
  GstElement *level;
  GstCaps *caps;

  GST_DEBUG("setup_element");
  level = gst_check_setup_element("ebur128");
...
static void cleanup_element(GstElement *element) {
...
GST_START_TEST(test_setup_and_teardown) {
  ...
  element = setup_element(S16_CAPS_STRING);
  ...
  ASSERT_OBJECT_REFCOUNT(element, "level", 1);
